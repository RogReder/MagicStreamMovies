...
func AdminReviewUpdate(client*mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Unable to extract user role"})
			return
		}
		...
		if movieId == "" {
			c.JSON(http.StatusBadGateway, gin.H{"error": "Movie id required"})
			return
		}
...
func GetReviewRanking(admin_review string, client*mongo.Client, c*gin.Context) (string, int, error) {
	...
	messages := []llms.MessageContent{
		llms.TextParts(llms.ChatMessageTypeSystem, basePrompt),
...
func GetRecommendedMovies(client*mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"errors": "user Id not found in context"})
			return
		}
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error fetching recommanded movies"})
			return
		}

		defer cursor.Close(ctx)

		var recommandedMovies []models.Movie

		if err := cursor.All(ctx, &recommandedMovies); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}

		c.JSON(http.StatusOK, recommandedMovies)
...
func GetUsersFavouriteGenres(userId string, client*mongo.Client, c*gin.Context) ([]string, error) {
	...
	favouriteArray, ok := result["favourite_genres"].(bson.A)
	...
	for _, item := range favouriteArray {
